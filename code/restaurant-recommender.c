...
#define MAX_RESTAURANTS 99       // max number of restaurants
#define MAX_CUISINE_LENGTH 29    // max length of restaurant cuisine
#define MAX_RESTAURANT_NAME 100  // max length of restaurant name
#define ID_LENGTH 6              // maximum length of restaurant id

#define MAX_PRICE_DIFF 20        // max price difference for 2 restaurants to be similar
#define MAX_DISTANCE_BETWEEN 30  // max distance difference for 2 restaurants to be similar
...
restaurant_t find_restaurant_with_smallest_avg_price(restaurant_records_t restaurant_records, int *num_restaurants);

/* stage two */
void stage_two(list_t *dining_records, restaurant_records_t restaurant_records, int num_restaurants);
int get_restaurant_idx(int restaurant_id, int num_restaurants, restaurant_records_t restaurant_records);
node_t *find_customer_data(customer_t customer, list_t *dining_record, int num_restaurants);
void add_customer_data(customer_t customer, list_t *dining_records, int restaurant_idx, int num_restaurants);
void print_dining_records(list_t *dining_records, int num_restaurants);

/* stage three */
void stage_three(int num_restaurants, list_t *dining_records, restaurant_records_t restaurant_records);
int is_recommended(int idx_restaurant_1, int idx_restaurant_2, restaurant_records_t restaurant_records);
...
//STAGE ONE
void stage_one(restaurant_records_t restaurant_records, int *num_restaurants) {
    /* read restaurant records from input data; put each record into the array `restaurant_records`. print total number of restaurant records, and the restaurant with smallest average price per head. */
    
    print_stage_header(1);
    
    // read each restaurant record into `restaurant_records` until end of restaurant records section
    for (int i = 0; read_one_restaurant_record(&restaurant_records[i]); i++) {
        *num_restaurants += 1;
    }
    
    restaurant_t restaurant_with_smallest_avg_price = find_restaurant_with_smallest_avg_price(restaurant_records, num_restaurants);
...
int read_one_restaurant_record(restaurant_t* restaurant) {
    /* initialize and return a restaurant_t object by extracting data from one restaurant record. */
...
restaurant_t find_restaurant_with_smallest_avg_price(restaurant_records_t restaurant_records, int* num_restaurants) {
    /* return restaurant in `restaurant_records` with smallest average price per head */
...
//STAGE TWO
void stage_two(list_t *dining_records, restaurant_records_t restaurant_records, int num_restaurants) {
    /* print the number of times each customer in the dining records visited each restaurant in the restaurant records in the form of an matrix */
    ...
    while(getchar() != EOF) {
        ...
        int restaurant_idx = get_restaurant_idx(restaurant_id, num_restaurants, restaurant_records);
        
        //  add extracted data as a node in `dining_records`
        add_customer_data(curr_customer, dining_records, restaurant_idx, num_restaurants);
    }
...
int get_restaurant_idx(int restaurant_id, int num_restaurants, restaurant_records_t restaurant_records) {
    ...
    for (int i = 0; i < num_restaurants; i++) {
        /* find which restaurant id in `restaurant_records` matches with `restaurant_id` */
...
void add_customer_data(customer_t customer, list_t *dining_records, int restaurant_idx, int num_restaurants) {
    /* add data extracted (`customer`) from a single dining record into `dining_records` */
    
   // node in `dining_records` to add the new data to
   node_t *insertion_point = find_customer_data(customer, dining_records, num_restaurants);
   
   // update `customer` data by adding the number of times the customer has visited the restaurant with index `restaurant_idx` by 1
...
node_t *find_customer_data(customer_t customer, list_t *dining_records, int num_restaurants) {
    /* return pointer of node which contains a customer's `customer` data. */
    
    node_t *temp = dining_records->head;
    
    /* if customer id in linked list of dining records, return pointer to node containing customer's data */
    while (temp != NULL) {
    ...
    /* if customer id not in linked list of dining records, initialize array to record number of times customer visited each restaurant */
...
//STAGE THREE
void stage_three(int num_restaurants, list_t *dining_records, restaurant_records_t restaurant_records) {
    /* for each customer C in `dining_records`, based on restaurants that C has visited before, identify unvisited restaurants that would be recommended to C. subtract corresponding restaurant_visits
    value by one for each recommended restaurant. */
    ...
    while (temp != NULL) {
        ...
        /* compare each restaurant customer has visited to restaurants customer have never visited */
        for (int i = 0; i < num_restaurants; i++) {
            if (curr_customer.restaurant_visits[i] > 0) {
                
                // if a restaurant is recommended, subtract value corresponding to the restaurant in `restaurant_visits` array by 1.
...
int is_recommended(int idx_restaurant_1, int idx_restaurant_2, restaurant_records_t restaurant_records) {
    /* evaluate whether the restaurant with index `idx_restaurant_2` would be recommended to a customer that visited the restaurant with index `idx_restaurant_1` in `restaurant_records`. return 1 if
    `restaurant_2` is recommended and 0 otherwise. */
    ...
    if (abs(price_diff) <= MAX_PRICE_DIFF) {
        return 1;
    }
    ...
    if (distance_between <= MAX_DISTANCE_BETWEEN) {
        return 1;
    }
...
double restaurant_distance(restaurant_t restaurant_1, restaurant_t restaurant_2) {
    ...
    return sqrt(pow((restaurant_1.x_coord - restaurant_2.x_coord), 2) + pow((restaurant_1.y_coord - restaurant_2.y_coord), 2));
}
...
void stage_four(list_t *dining_records, int num_restaurants, restaurant_records_t restaurant_records) {
    ...
    while (temp != NULL) {
        ...
        find_2_most_similar(curr_customer, dining_records, num_restaurants, top2_most_similar);
        
        /* update `curr_customer`'s restaurant_visits array to add recommendations based on the top 2 most similar customers' restaurant_visits array */
        update_recommendation(num_restaurants, curr_customer, top2_most_similar);
...
void find_2_most_similar(customer_t customer, list_t *dining_records, int num_restaurants, customer_t top2_most_similar[2]) {
    /* find 2 customers in `dining_records` with highest similarity scores to `customer` and store them in `top_2_most_similar`. */
    ...
    while (temp != NULL) {
        ...
        if (strcmp(curr_customer.id, customer.id)) {
            ...
            int similarity_score = get_similarity_score(customer, curr_customer, num_restaurants);
            
            /* if `curr_customer`'s similarity score is higher than the top1, remove the current top2 customer from `top2_most_similar`. move the top1 customer to top2 and store `curr_customer` as top1 */
            ...
            /* if `curr_customer`'s similarity score is higher than top2 but less than top1, replace the top2 customer with `curr_customer` */
...
int get_similarity_score(customer_t customer_1, customer_t customer_2, int num_restaurants) {
    ...
    for (int i = 0; i < num_restaurants; i++) {
        ...
        /* if both customers have visited the same restuarant with index i, add the product of their restaurant_visits[i] value to `similarity_score` */
        if (customer_1.restaurant_visits[i] > 0 && 
            customer_2.restaurant_visits[i] > 0) {
            similarity_score += (customer_1.restaurant_visits[i] * customer_2.restaurant_visits[i]);
        }
    }
...
void update_recommendation(int num_restaurants, customer_t customer, customer_t top2_most_similar[2]) {
    /* for each customer in `top_2_most_similar` with similarity score more than 0, recommend restaurants unvisited by A, that have been visited by the top1 and/or top2 most similar customers (if kept).
    for each recommended restaurant, subtract corresponding value in restaurant_visits array by one.*/
